  ...
  static struct DataStackElem {
    ...
    void set(T, _Type t = _Type.NODATA)(in T data) {
      static if (is(T == int)) {
        ...
        static assert(false, "DataStackElem: Unsupported data type.");
      }
    }
...
void onMarkedFiles(scope void delegate(ViewIndex v) dg)
{
  int m = dataStack.pop!MARK().ifNull(cur);
  int beg, end;
  if (m > cur) {
    beg = cur;
    end = m + 1;
...
#line 1212 "sf.nw"
case 's':
  onMarkedFiles(v => fileList.select(v));
  break;
case 'S':
  onMarkedFiles(v => fileList.deselect(v));
  break;
case 't':
  onMarkedFiles(v => {
    if (fileList.isSelected(v))
      fileList.deselect(v);
    else
      fileList.select(v);
    }());
