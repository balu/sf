...
\nwendcode{}\nwbegindocs{53}We now define the main UI structure. The {\Tt{}alias\ sink\nwendquote} parameter is
the sink function that consumes the range of selected files. It is
called if the user pressed 'q' to exit the application.

\nwenddocs{}\nwbegincode{54}\moddef{MainUI definition}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{55}The echo window can be accessed programmatically in the following
...
\nwenddocs{}\nwbegincode{56}\moddef{Echo area functions}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
auto readKey()
\{
  dchar c;
  enforce(
...
\nwendcode{}\nwbegindocs{57}Now we describe our command processing loop. The input to the
...
\nwenddocs{}\nwbegincode{58}\moddef{Data stack definition}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{59}The words in the programming language are grouped into data or
...
\nwenddocs{}\nwbegincode{60}\moddef{States of the command processor}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
enum \{
  START,
  READ_CH,
  READ_STR,
...
\nwendcode{}\nwbegindocs{61}The character {\Tt{}c\nwendquote} is used to read the current character input by
the user.

\nwenddocs{}\nwbegincode{62}\moddef{Local variables for command processor}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
dchar c;

\nwendcode{}\nwbegindocs{63}The command processor works in iterations. In each iteration, it
...
\nwenddocs{}\nwbegincode{64}\moddef{Setup state for the command processor}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
c   = cast(dchar)readKey();

\nwendcode{}\nwbegindocs{65}The start state simply looks at the key and delegates to the
appropriate state to handle the key.

\nwenddocs{}\nwbegincode{66}\moddef{Start state}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
case START:
  clearEcho();
  if (c.isWhite) \{
    state = START;
...
\nwendcode{}\nwbegindocs{67}Processing a data item is straight-forward. Simply read it and push
it onto the stack. The variables {\Tt{}n\nwendquote} and {\Tt{}s\nwendquote} are used to keep
track of the number and string input the user.

\nwenddocs{}\nwbegincode{68}\moddef{Local variables for command processor}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
int n;
string s;

\nwendcode{}\nwbegincode{69}\moddef{Read data items}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
case READ_CH:
  writeChar(c);
  dataStack.push!CH(c);
  state = START;
...
\nwendcode{}\nwbegindocs{70}Now we process the commands. All commands are single letter
...
\nwenddocs{}\nwbegincode{71}\moddef{Read commands}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
case READ_COMMAND:
  switch (c) \{
...
\nwendcode{}\nwbegindocs{72}The reverse command.

\nwenddocs{}\nwbegincode{73}\moddef{Handle commands}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
case 'r':
  fileList.reverse(); break;

\nwendcode{}\nwbegindocs{74}Handle motion commands. This part illustrates why mark and number
...
\nwendcode{}\nwbegindocs{76}For the selection commands, we first check the stack to see whether
...
\nwenddocs{}\nwbegincode{77}\moddef{Local variables for command processor}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
int m, beg, end;

\nwendcode{}\nwbegincode{78}\moddef{Obtain range of files}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{80}The sort command accepts an argument on the stack that specifies the
...
\nwendcode{}\nwbegindocs{82}The glob command simply delegates the actual glob search to the
...
\nwendcode{}\nwbegincode{86}\moddef{Handle commands}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
default: break; // Unknown command letter.

\nwendcode{}\nwbegindocs{87}We define some commands to manipulate the data stack. The peek
...
\nwendcode{}\nwbegindocs{89}The following are convenience functions used by our command
processor.

\nwenddocs{}\nwbegincode{90}\moddef{Convenience functions for command processor}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{91}Finally, we put everything together to make our command processor.

\nwenddocs{}\nwbegincode{92}\moddef{loop function}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
void loop()
\{
...
\nwendcode{}\nwbegindocs{93}We will now describe our main redrawing logic. The current file has
...
\nwenddocs{}\nwbegincode{94}\moddef{MainUI variables}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
ViewIndex first = ViewIndex(0);

\nwendcode{}\nwbegincode{95}\moddef{Display the appropriate portion of file list pad}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{96}Now we will write some output routines that print the files selected
into standard output in various common formats.

\nwenddocs{}\nwbegincode{97}\moddef{Output routines}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{98}Our application.

\nwenddocs{}\nwbegincode{99}\moddef{sf.d}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegincode{100}\moddef{Import statements}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{101}Now we define the various index and array types. The type
...
\nwenddocs{}\nwbegincode{102}\moddef{Definitions for index and array types}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{103}We will also allow to convert an index into an {\Tt{}int\nwendquote}.

\nwenddocs{}\nwbegincode{104}\moddef{Definitions for index and array types}\plusendmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{105}Some convenience functions.

\nwenddocs{}\nwbegincode{106}\moddef{Convenience functions}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{107}Some useful aliases.

\nwenddocs{}\nwbegincode{108}\moddef{Useful aliases}\endmoddef\nwstartdeflinemarkup\nwenddeflinemarkup
...
\nwendcode{}\nwbegindocs{109}\section*{Some implementation notes} There are many casts from
